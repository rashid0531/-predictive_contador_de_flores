...
def save_annotated_image(canola_image):
    canola_image.getFlowerCountObject().saveDetectedBlobs(canola_image.getPath())
...
def run_fc_jobs_in_parallel(input_path,output_path,SparkContext):
    ...
    pipeline_rdd = histResult_rdd.zipWithIndex() \
        ...
        .foreach(lambda x: saveDetectedBlobs(x, output_path))
    #.foreach(lambda x: (x.getPath(), x.saveDetectedBlobs(x.getPath(),output_path)))


def saveDetectedBlobs(img,output_path):

    imagename = img.getPath()
    blobs = img.getFlowerCountObject().getBlobs()
    assert blobs is not None
    image = np.copy(img.readImage())

    print("------ TYPE : ",type(image), "----")

    for x, y, r in blobs:
         cv2.circle(image,center=(int(y), int(x)),radius=1,color=(0,0,255),thickness=2)

    output_prefix = "annotated"+imagename.split("/")[-1]

    # # print("------output : ",output, "----" , "prefix : ",output_prefix)

    if not os.path.exists(output_path):
         os.makedirs(output_path)

    cv2.imwrite(output_path + output_prefix, image)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(output_path):
        print("output folder doesn't exit.")
